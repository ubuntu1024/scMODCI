...
from .scmodci import  SCMODCIModel
...
@logged
def scMODCI_trainer(
        adatas: Mapping[str, AnnData], graph: nx.Graph, model: type = SCMODCIModel,
        init_kws: Kws = None, compile_kws: Kws = None, fit_kws: Kws = None,
        balance_kws: Kws = None
) -> SCMODCIModel:
    r"""
    Fit SCMODCIModel model to integrate single-cell multi-omics data
    ...
    model
        ...
        {:class:`scMODCI.models.scmodci.SCMODCIModel`}
    init_kws
        Model initialization keyword arguments
        (see the constructor of the ``model`` class,
        either :class:`scMODCI.models.scmodci.SCMODCIModel`)
    compile_kws
        Model compile keyword arguments
        (see the ``compile`` method of the ``model`` class,
        either :meth:`scMODCI.models.scmodci.SCMODCIModel.compile`)
    fit_kws
        Model fitting keyword arguments
        (see :meth:`scMODCI.models.scmodci.SCMODCIModel.fit`)
    balance_kws
        Balancing weight estimation keyword arguments
        (see :func:`scMODCI.data.estimate_balancing_weight`)
    ...
    scMODCI_trainer.logger.info("Pretraining scMODCI model...")
    ...
    scMODCI_trainer.logger.info("Estimating balancing weight...")
    ...
    scMODCI_trainer.logger.info("Fine-tuning scMODCI model...")
    ...
    scMODCI_trainer.logger.debug("Increasing random seed by 1 to prevent idential data order...")
