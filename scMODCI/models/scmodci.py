...
from .MODCI import SCMODCI, SCMODCITrainer
...
class ZEncoder(torch.nn.Module):
    ...
    def __init__(
        ...
        self.loc = KANLinear(ptr_dim, out_features)
        self.std_lin = KANLinear(ptr_dim, out_features)
...
class SCMODCI(SCMODCI):

    r"""
    SCMODCI network for single-cell multi-omics data integration
    ...
    u2z
        Data mapping (indexed by modality name)
    z2u
        Data De-mapping (indexed by modality name)
    u2x
    ...
    def __init__(
        ...
        super().__init__(g2v, v2g, x2u,u2z,z2u, u2x, idx, du, prior)
...
]  # Specifies the data format of input to SCMODCITrainer.compute_losses


@logged
class SCMODCITrainer(SCMODCITrainer):

    r"""
    Trainer for :class:`SCMODCI`

    Parameters
    ----------
    net
        :class:`SCMODCI` network to be trained
    lam_data
        Data weight
    lam_kl
        KL weight
    ...
    **kwargs
    ...
    def __init__(
            self, net: SCMODCI, lam_data: float = None, lam_kl: float = None,
    ...
    def compute_losses(
        ...
        vsamp = v.rsample()
        for k in net.keys:
            z[k] = net.u2z(u[k].mean)
            u1[k] = net.z2u(z[k].mean)
        ...
        dsc_loss_models = F.cross_entropy(net.du(u_cat, xbch_cat), xflag_cat, reduction="none")
        dsc_loss_models = (dsc_loss_models * xdwt_cat).sum() / xdwt_cat.numel()
        dsc_loss=dsc_loss_models
        if dsc_only:
        ...
        x_u1_nll = {
            k: -net.u2x[k](
                usamp1[k],vsamp[getattr(net, f"{k}_idx")], xbch[k], l[k]
            ).log_prob(x[k]).mean()
            for k in net.keys
        }
        x_u1_kl = {
        ...
        # Calculate the contrast loss (compare U and u1 of the same cell)
        contrast_loss = 0.0
        criterion = SupConLoss(temperature=0.01, cuda_device='0')
        for k in net.keys:
            u_norm = F.normalize(usamp[k],  dim=1)
            u1_norm = F.normalize(z[k].rsample(),  dim=1)
            loss=criterion(u_norm,u1_norm)
            contrast_loss += loss

        contrast_loss /= len(net.keys)


        # ========== neighbor consistency loss部分 ==========
        neighbor_loss = 0.0
        for k in net.keys:
            zsamp = F.normalize(z[k].rsample(), dim=1)
            xrepsamp = F.normalize(xrep[k], dim=1)
            sim_z = torch.matmul(zsamp, zsamp.T)
            sim_xrep = torch.matmul(xrepsamp, xrepsamp.T)

            neighbor_loss += F.mse_loss(sim_z, sim_xrep)

        neighbor_loss = neighbor_loss / len(net.keys)

        x_u1_elbo = {
            k: x_u1_nll[k] + self.lam_kl * x_u1_kl[k]
            for k in net.keys
        }
        x_u1_nll_sum = sum(self.modality_weight[k] * x_u1_elbo[k] for k in net.keys)

        vae_loss = self.lam_data * (x_elbo_sum+x_u1_nll_sum) \
            + self.lam_graph * len(net.keys) * g_elbo \
            + self.lam_sup * sup_loss +0.1*neighbor_loss+0.1 * contrast_loss
...
@logged
class SCMODCIModel(Model):

    r"""
    SCMODCI model for single-cell multi-omics data integration
    ...
    def __init__(
        ...
        self.modalities, idx, x2u, u2z, z2u, u2x, all_ct = {}, {}, {}, {}, {}, {}, set()
        ...
        super().__init__(
            ...
            g2v, v2g, x2u, u2z, z2u, u2x, idx, du, prior,
    ...
    def adopt_pretrained_model(
            self, source: "SCMODCIModel", submodule: Optional[str] = None
    ) -> None:
        r"""
    ...
    def decode_data(
        ...
        for u_, b_, l_ in data_loader:
            ...
            result.append(decoder(z2u(u_), v, b_, l_).mean.detach().cpu())
        return torch.cat(result).numpy()
    ...
    def __repr__(self) -> str:
        return (
            f"SCMODCI model with the following network and trainer:\n\n"
            f"{repr(self.net)}\n\n"
            f"{repr(self.trainer)}\n"
        )
